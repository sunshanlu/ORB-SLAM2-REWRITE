...
static void computeOrbDescriptor(const cv::KeyPoint &kpt, const cv::Mat &img, const cv::Point *pattern, uchar *desc) {
    float angle = kpt.angle / 180.0 * CV_PI;
...
int main(int argc, char **argv) {
    ...
    cv::Ptr<cv::FeatureDetector> detectorCV = cv::ORB::create();

    std::vector<cv::KeyPoint> keypoints0, keypoints1, keypoints2, keypoints3;
    std::vector<int> u_max;
    detectorCV->detect(image, keypoints0);

    // 以16为边界，剔除在边界以外的特征点
    for (const auto &kp : keypoints0) {
        if (kp.pt.x > 16 && kp.pt.x < image.cols - 16 && kp.pt.y > 16 && kp.pt.y < image.rows - 16) {
            keypoints1.push_back(kp);
        }
    }

    std::copy(keypoints1.begin(), keypoints1.end(), std::back_inserter(keypoints2));
    std::copy(keypoints1.begin(), keypoints1.end(), std::back_inserter(keypoints3));

    detector->centroidMethod(image, keypoints2);
    pixelCirclePos(15, u_max);
